	...
	<div class="container">
		<div class="row">
			<table class="table table-striped" style="text-align: center; border: 1px-solid #dddddd">
				...
				<tbody>
					...
					<tr style="line-height: 50px;">
						...
						<td style="line-height: 50px;"><%= Integer.toString(list.get(i).getClassCode()) %></td>
						<td style="line-height: 50px;"><a href="view.jsp?classCode=<%= list.get(i).getClassCode() %>"><%=list.get(i).getClassName() %></a></td>
						<td style="line-height: 50px;"><%= list.get(i).getProfessorName() %></td>
						<td style="line-height: 50px;"><%= list.get(i).getCourse() %></td>
						<td style="line-height: 50px;"><%= list.get(i).getCredit() %></td>
						<td style="line-height: 50px;"><a href="registerAction.jsp?classCode=<%= list.get(i).getClassCode() %>" class="btn btn-primary center">수강신청</a></td>
					</tr>
					<%
						}
					%>
			...
			<%
				if(pageNumber != 1){
			%>
				<a href="registerCourse.jsp?pageNumber=<%=pageNumber - 1%>" class="btn btn-success btn-arraw-left">이전</a>
			<%
				} if(lecturePlanDAO.nextPage(pageNumber + 1)){
			%>
				<a href="registerCourse.jsp?pageNumber=<%=pageNumber + 1%>" class="btn btn-success btn-arraw-left">다음</a>
			<%
				}
			%>
		</div>
